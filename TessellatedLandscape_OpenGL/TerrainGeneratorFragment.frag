...
int primes[30] = { 995615039, 600173719, 701464987,
...
{   
	...
	// NB: InterpolatedNoise non usava floor per la parte intera, scazzando di brutto in caso di coordinate negative.
	
	// [-1, 1] -> [0, 2] -> [0, resolution] -> [offset*resolution , (offset+1) * resolution] 
   float red = (perlin((Coords.x + 1 + 2 * offsetX) * resolution / 2, (Coords.y + 1 + 2*offsetY)*resolution/2));

   // Il rumore è mappato in [-0.5, 0.5] ma i canali colore fanno [0, 1]
   gl_FragColor = vec4(red+0.5, 0.0, 0.0, 1.0);
}